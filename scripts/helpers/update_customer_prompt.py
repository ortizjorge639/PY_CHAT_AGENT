"""Update STATUS BUSINESS REASON + RESPONSE STYLE in kernel_customer.py with customer-provided interpretations."""

with open("agent/kernel_customer.py", "r", encoding="utf-8") as f:
    content = f.read()

# Markers
start_marker = "STATUS \u2192 BUSINESS REASON (use when explaining statuses)"
end_marker = "FILE EXPORT RULES"
...
if idx_start == -1:
    ...
    exit(1)
if idx_end == -1:
    print("ERROR: Could not find FILE EXPORT RULES marker")
    exit(1)

# Go back to the dashes line before STATUS
line_before_start = content.rfind("----------------------------------------------------------------\n", 0, idx_start)
# Go back to the dashes line before FILE EXPORT RULES
...
with open("agent/kernel_customer.py", "w", encoding="utf-8") as f:
    f.write(content)

print("SUCCESS - kernel_customer.py updated")
